 ...
 */
import type { PlaygroundWidgetDesign } from "@/modules/playground/types";
import {
  DEFAULT_WIDGET_DESIGN,
  DEFAULT_DARK_WIDGET_DESIGN,
} from "@/modules/playground/types";

type PresetInput = Omit<
  Partial<PlaygroundWidgetDesign>,
  "position" | "customX" | "customY" | "width" | "height" | "borderRadius"
>;

function lightPreset(overrides: PresetInput): PlaygroundWidgetDesign {
  return { ...DEFAULT_WIDGET_DESIGN, ...overrides };
}

function darkPreset(overrides: PresetInput): PlaygroundWidgetDesign {
  return { ...DEFAULT_DARK_WIDGET_DESIGN, ...overrides };
}

export interface WidgetColorPreset {
  name: string;
  design: PlaygroundWidgetDesign;
}
