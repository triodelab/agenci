      ...
      <div className="bg-[#010102]">
        <div className="mx-auto max-w-[720px] px-6 py-20 md:py-24 xl:px-8">
          ...
          >
            ...
            <ol className="mt-4 list-decimal space-y-2 pl-5 text-[14px] leading-[1.5] text-[#8a8f98] marker:font-medium marker:text-[#5e6ad2]">
              {toc.map((item) => (
                <li key={item.id}>
...
function ListItem({ label, children }: { label: string; children: ReactNode }) {
  return (
    <li className="flex items-start gap-3 text-[14px] leading-[1.6] text-[#d0d6e0]">
      ...
      <span>
        {label && <span className="font-medium text-[#f7f8f8]">{label}: </span>}
        {children}
      </span>
    </li>
  );
