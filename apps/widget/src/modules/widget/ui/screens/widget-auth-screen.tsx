...
export const WidgetAuthScreen = () => {
  ...
  const startSession = async (
    ...
    const { contactSessionId } = await api.public.contactSessions.create({
      organizationId,
      ...(agentId ? { agentId } : {}),
      ...(name ? { name } : {}),
      ...(email ? { email } : {}),
    ...
    // Booking/voice (the only reasons to show the selection screen) aren't
    // wired to the server yet — go straight to chat with a fresh thread.
  ...
  const onSkip = async () => {
    await startSession(undefined, undefined, true);
  };
    ...
    <>
      ...
      <Form {...form}>
        ...
        >
          ...
          >
            Ved å fortsette godtar du at navn og e-post lagres for å håndtere
            din henvendelse.{" "}
            <a
