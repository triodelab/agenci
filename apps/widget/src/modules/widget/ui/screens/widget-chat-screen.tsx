...
export const WidgetChatScreen = () => {
  ...
  const agentIdFromUrl = useAtomValue(agentIdAtom);
  ...
  // Explicit `agentId` from the embed wins; otherwise the server-resolved default agent.
  const agentId = agentIdFromUrl ?? widgetSettings?.agentId ?? null;
  // Customer's logo (from onboarding branding) as the assistant avatar.
  const assistantAvatarUrl = widgetSettings?.faviconUrl ?? "/AgenciLogo.png";
  ...
  const handleIdentitySubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!contactSessionId || !identityName.trim() || !identityEmail.trim())
      return;
    setIsIdentitySubmitting(true);
    try {
      await api.public.contactSessions.updateIdentity({
  ...
  const handleDeleteHistory = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.public.contactSessions.deleteMySession();
    } catch {
    ...
    <div className="relative flex h-full min-h-0 min-w-0 flex-1 flex-col">
      ...
      <AIConversation className="min-h-0 min-w-0 flex-1 bg-[var(--widget-bg,#fff)]">
        <AIConversationContent className="px-3 pb-2 pt-1 sm:px-4">
          {messages.map((raw) => {
            // The greeting follows the configured text (also live in preview).
            const message =
              raw.id === GREETING.id && widgetSettings?.greeting?.trim()
